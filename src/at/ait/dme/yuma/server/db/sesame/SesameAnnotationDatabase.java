package at.ait.dme.yuma.server.db.sesame;
...
public class SesameAnnotationDatabase extends AnnotationDatabase {
	...
	private static final String ANNOTATION_LINKED_TO_RESOURCES = 
		ANNOTATION_REL_NS + "isLinkedToResources";
	...
	public String findAnnotations(String term) throws AnnotationDatabaseException {
		String query = 
			"CONSTRUCT {?x <" + HTTP_BODY + "> ?body} " + 
				"WHERE {{?x <" + HTTP_BODY + "> ?body ." +
							"FILTER regex(str(?body), \"" + term + "\", \"i\")} " +	
					   "UNION {OPTIONAL" + 
					   "{?x <" + HTTP_BODY + "> ?body ." +
					   " ?y <" + ANNOTATION_BODY + "> ?x ." +
					   " ?y <" + ANNOTATION_LINKED_TO_RESOURCES + "> ?links . " +
							"FILTER regex(str(?links), \"" + term + "\", \"i\")}}" +									
				"}";
