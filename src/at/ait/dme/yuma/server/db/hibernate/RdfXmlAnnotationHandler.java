...
public class RdfXmlAnnotationHandler implements RDFHandler {
	...
	private Map<Long, ArrayList<Annotation>> allReplies = new HashMap<Long, ArrayList<Annotation>>();
	...
	public void handleStatement(Statement statement) throws RDFHandlerException {
		try {
			// we found a new annotation when we find a new resource that does
			// not describe the body of an annotation (the body could be an embedded resource
			// and is described using predicates from http://www.w3.org/2000/10/annotation-ns#,
			// see also http://www.w3.org/2001/Annotea/User/Protocol.html#PostABody)
			if (currentResource == null
					|| (currentResource != null && !currentResource.equals(statement.getSubject()) && !statement
							.getPredicate().getNamespace().equals(RdfXmlAnnotationBuilder.HTTP_NS))) {
	...
	private void createAnnotation() throws JDOMException, IOException, ParseException {
		...
		if (objectId != null)
			annotation.setObjectUrl(objectId.stringValue());

		Value extObjectId = currentStatements.get(RdfXmlAnnotationBuilder.ANNOTATION_LINKED_TO);
		if (extObjectId != null)
			annotation.setExternalObjectId(extObjectId.stringValue());
		...
		for (Value link : links)
			annotation.addLink(link.stringValue());
	}
	...
	private static Long readAnnotationId(String url) {
		Long id = null;
		url = url.replace(Config.getInstance().getAnnotationBaseUrl(), "");
		try {
			id = Long.parseLong(url);			
		} catch (NumberFormatException nfe) {			
			// workaround to import annotations from FAST
			try {
				url = url.substring(url.length()-12);			
				id = Long.parseLong(url, 16);
			} catch (NumberFormatException nfe2) {
				id = new Long(url.hashCode());
			}
		}
		return id;
	}
