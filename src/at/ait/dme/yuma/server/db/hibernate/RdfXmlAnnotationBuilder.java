...
public class RdfXmlAnnotationBuilder {
	...
	public static String toRdfXml(final Annotation annotation) throws RepositoryException,
		...
		return toRdfXml(new ArrayList<Annotation>() {{ add(annotation); }}, true);	
	}

	public static String toRdfXml(final Annotation annotation, boolean processReplies) 
			throws RepositoryException, RDFHandlerException, IOException {		
		return toRdfXml(new ArrayList<Annotation>() {{ add(annotation); }}, processReplies);	
	}
	
	public static String toRdfXml(Collection<Annotation> annotations)
			throws RepositoryException, RDFHandlerException, IOException {
		return toRdfXml(annotations, true);
	}
	 ...
	 */
	public static String toRdfXml(Collection<Annotation> annotations, boolean processReplies)
			throws RepositoryException, RDFHandlerException, IOException {
		...
		for(Annotation annotation : annotations)
			toRdf(con,valueFactory,annotation, processReplies);
	...
	private static void toRdf(RepositoryConnection con, ValueFactory vFactory,
			Annotation annotation, boolean processReplies) throws RepositoryException,
			RDFHandlerException, IOException {	
	    ...
	    // replies
	    if(annotation.getReplies()!=null && processReplies) {
	    	for(Object reply : annotation.getReplies()) toRdf(con,vFactory,(Annotation)reply, true);
	    }
	}
	...
	public static Annotation fromRdfXml(String rdfXml) throws RDFParseException, 
		...
		return parseRdfXml(rdfXml).getAnnotations().get(0);
	}
	
	/**
	 * convert a rdf/xml representation into an list of <code>Annotation</code>
	 * 
	 * @param rdfXml
	 * @return annotation
	 * @throws RDFParseException
	 ...
	 * @throws IOException
	 */
	public static List<Annotation> fromRdfXmlAsList(String rdfXml) throws RDFParseException,
			RDFHandlerException, IOException {
	
		return parseRdfXml(rdfXml).getAnnotations();
	}
	 ...
	 * @throws IOException
	 */
	private static RdfXmlAnnotationHandler parseRdfXml(String rdfXml) throws RDFParseException, 
			RDFHandlerException, IOException {		 
